...
public class BestTimeToBuyAndSellStockWithCooldown {
    private enum STAGE { BUY, COOLDOWN, SELL }
    ...
    public int maxProfit(int[] prices) {
        this.prices = prices;
        return maxProfit(STAGE.BUY, 0);
    }

    private int maxProfit(STAGE stage, int index) {
        if (prices.length == index)
            return 0;

        final String key = stage.toString() + index;
        if (memo.containsKey(key)) return memo.get(key);

        int ret;
        if (stage.equals(STAGE.BUY)) {
            ret = Math.max(
                maxProfit(STAGE.SELL, index + 1) - prices[index],
                maxProfit(STAGE.BUY, index + 1)
            );
        }
        else if (stage.equals(STAGE.SELL)) {
            ret = Math.max(
                maxProfit(STAGE.COOLDOWN, index + 1) + prices[index],
                maxProfit(STAGE.SELL, index + 1)
            );
        }
        else { // STAGE.COOLDOWN
            ret = maxProfit(STAGE.BUY, index + 1);
        }
    ...
    @Nested
    class UnitTest {
        private final BestTimeToBuyAndSellStockWithCooldown sut = new BestTimeToBuyAndSellStockWithCooldown();

        @Test
        public void example1() {
            int[] input = {1,2,3,0,2};
            int expected = 3;
            assertEquals(expected, sut.maxProfit(input));
        }
